...
Deno.serve(async (req) => {
  ...
  const { data: room, error: roomError } = await admin.from("event_rooms")
    .select("id,host_id,status,is_private").eq("id", body.roomName).maybeSingle();
  ...
  const { data: attendee } = await admin.from("event_attendees").select("status,can_stream")
    .eq("event_room_id", room.id).eq("user_id", userData.user.id).maybeSingle();
  ...
  try {
    ...
    return jsonResponse({
      ...
      roomName: room.id,
      participantIdentity: userData.user.id,
      canPublish: mayPublish,
    });
  } catch {
