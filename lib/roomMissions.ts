...
export type EncounterResultStatus = "valid" | "self_scan" | "wrong_mission" | "wrong_animal" | "duplicate" | "expired" | "mission_ended" | "invalid";
export type MissionEncounterResult = { status: EncounterResultStatus; progress?: number; target_encounters?: number; completed?: boolean };
export type AnimalPackHostResults = {
  participant_count: number;
  completed_count: number;
  completed_participants: { identity_id: string; display_name: string; avatar_url: string | null; completed_at: string }[];
};

export type MissionOperationalStatus = "healthy" | "waiting_for_participants" | "needs_people" | "imbalanced" | "ended";
...
export type MissionCompletedParticipants = {
  ...
  offset: number;
  has_more: boolean;
  participants: { identity_id: string; display_name: string; avatar_url: string | null; assignment_key: string | null; completed_at: string }[];
};

export const animalDetails: Record<string, { singular: string; plural: string }> = {
  "🐸": { singular: "frog", plural: "frogs" }, "🦁": { singular: "lion", plural: "lions" },
  "🐼": { singular: "panda", plural: "pandas" }, "🦊": { singular: "fox", plural: "foxes" },
  "🐵": { singular: "monkey", plural: "monkeys" }, "🐙": { singular: "octopus", plural: "octopuses" },
  "🐯": { singular: "tiger", plural: "tigers" }, "🐨": { singular: "koala", plural: "koalas" },
  "🐧": { singular: "penguin", plural: "penguins" }, "🐰": { singular: "rabbit", plural: "rabbits" },
  "🐺": { singular: "wolf", plural: "wolves" }, "🦄": { singular: "unicorn", plural: "unicorns" },
};
...
export function normalizeRoomMissionInput(input: RoomMissionInput) {
  ...
  if (title.length > 120) throw new Error("Mission title must be 120 characters or fewer.");
...
export async function getActiveRoomMission(supabase: SupabaseClient, roomId: string) {
  const { data, error } = await supabase.rpc("get_active_room_mission", { p_room_id: roomId });
  if (error) throw new Error(error.message);

  const mission = firstRow<RoomMission>(data);
  return mission ? { ...mission, completion_count: Number(mission.completion_count ?? 0), participant_count: Number(mission.participant_count ?? 0) } : null;
}

export async function getRoomMissionHistory(supabase: SupabaseClient, roomId: string, limit = 6) {
  ...
  return ((data ?? []) as RoomMissionHistoryItem[]).map((item) => ({
    ...item,
    completion_count: Number(item.completion_count ?? 0),
...
export async function endRoomMission(supabase: SupabaseClient, missionId: string) {
  const { data, error } = await supabase.rpc("end_room_mission", { p_mission_id: missionId });
  if (error) throw new Error(error.message);
  return firstRow<RoomMission>(data);
}

export async function completeRoomMission(supabase: SupabaseClient, missionId: string) {
  const { data, error } = await supabase.rpc("complete_room_mission", { p_mission_id: missionId });
  if (error) throw new Error(error.message);
  return firstRow<{ id: string; completed_at: string }>(data);
}

export async function verifyMemoryMissionCompletion(supabase: SupabaseClient, missionId: string, memoryId: string) {
  const { data, error } = await supabase.rpc("verify_memory_mission_completion", { p_mission_id: missionId, p_memory_id: memoryId });
  if (error) throw new Error(error.message);
...
export async function joinAnimalPackMission(supabase: SupabaseClient, missionId: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("join_animal_pack_mission", { p_mission_id: missionId, p_guest_token: guestToken ?? null });
  if (error) throw new Error(error.message);
  return data as { assignment_key: string };
}

export async function getMyAnimalPackState(supabase: SupabaseClient, missionId: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("get_my_animal_pack_state", { p_mission_id: missionId, p_guest_token: guestToken ?? null });
  if (error) throw new Error(error.message);
  return (data as AnimalPackState | null) ?? null;
}

export async function createMissionEncounterToken(supabase: SupabaseClient, missionId: string, guestToken?: string | null) {
...
export async function redeemMissionEncounterToken(supabase: SupabaseClient, missionId: string, tokenOrCode: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("redeem_mission_encounter_token", { p_mission_id: missionId, p_token_or_code: tokenOrCode, p_guest_token: guestToken ?? null });
  if (error) throw new Error(error.message);
...
export async function getAnimalPackHostResults(supabase: SupabaseClient, missionId: string) {
  const { data, error } = await supabase.rpc("get_animal_pack_host_results", { p_mission_id: missionId });
  if (error) throw new Error(error.message);
  const result = data as AnimalPackHostResults;
  return { ...result, participant_count: Number(result.participant_count ?? 0), completed_count: Number(result.completed_count ?? 0), completed_participants: result.completed_participants ?? [] };
}

export async function getMissionOperationsDashboard(supabase: SupabaseClient, missionId: string) {
  const { data, error } = await supabase.rpc("get_mission_operations_dashboard", { p_mission_id: missionId });
  if (error) throw new Error(error.message);
  return data as MissionOperationsDashboard;
}

export async function getMissionCompletedParticipants(supabase: SupabaseClient, missionId: string, limit = 100, offset = 0) {
  const { data, error } = await supabase.rpc("get_mission_completed_participants", { p_mission_id: missionId, p_limit: limit, p_offset: offset });
  if (error) throw new Error(error.message);
