...
import { createRecapMediaSignedUrl, type RoomRecapMedia } from "./recapMedia";
...
export async function resolveMyEventRecaps() {
  const { error } = await supabase.rpc("resolve_my_event_recaps");
  if (error) throw new Error(error.message);
}

export async function getEventRecap(roomId: string): Promise<EventRecap> {
  ...
  return {
    ...
    host_media: context.host_media ? await createRecapMediaSignedUrl(context.host_media) : null,
  };
}
