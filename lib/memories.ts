...
export async function getRoomMemories(roomId: string): Promise<RoomMemory[]> {
  ...
  const memories = Array.isArray(data)
    ? data.map(normalizeRoomMemory).filter((memory) => memory.id)
    : [];
  const savedIds = await getSavedRoomMemoryIds(memories.map((memory) => memory.id));
...
export async function getSavedRoomMemoryIds(memoryIds: string[]): Promise<Set<string>> {
...
export async function saveRoomMemory(memoryId: string): Promise<void> {
...
export async function unsaveRoomMemory(memoryId: string): Promise<void> {
...
export async function getMySavedMemoryGroups(): Promise<SavedMemoryGroup[]> {
