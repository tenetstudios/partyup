...
export type WildFaction = { key: string; label: string; emoji: string; color?: string };
...
export type WildMatchState = { progress: number; required_matches: number; completed: boolean; eligible: boolean; verification_type: "match_faction"; match_relationship: "opposing_faction"; verified_match_count: number; mission_completion_count: number; mission_active: boolean };
...
function normalizeState(value: unknown): WildRoomState {
  ...
  return {
    ...
    territories: (state.territories ?? []).map((territory) => ({
      ...territory,
      influence: Object.fromEntries(Object.entries(territory.influence ?? {}).map(([key, amount]) => [key, Number(amount ?? 0)])),
    })),
...
export async function getWildRoomState(supabase: SupabaseClient, roomId: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("get_wild_room_state", { p_room_id: roomId, p_guest_token: guestToken ?? null });
...
export async function enterWildGame(supabase: SupabaseClient, gameId: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("enter_wild_game", { p_game_id: gameId, p_guest_token: guestToken ?? null });
  if (error) throw new Error(error.message);
...
export async function publishWildMission(
  ...
  const { data, error } = await supabase.rpc(memoryVerification ? "publish_wild_memory_mission" : matchVerification ? "publish_wild_match_mission" : "publish_wild_faction_mission", {
    ...
    ...(memoryVerification ? { p_required_media_type: input.requiredMediaType ?? "any" } : matchVerification ? { p_required_matches: input.requiredMatches ?? 2 } : {
...
export async function getWildMatchState(supabase: SupabaseClient, missionId: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("get_my_wild_match_state", { p_mission_id: missionId, p_guest_token: guestToken ?? null });
  if (error) throw new Error(error.message);
  const state = data as WildMatchState;
  return { ...state, progress: Number(state.progress ?? 0), required_matches: Number(state.required_matches ?? 1), verified_match_count: Number(state.verified_match_count ?? 0), mission_completion_count: Number(state.mission_completion_count ?? 0) };
}
...
export async function completeWildMission(supabase: SupabaseClient, missionId: string, guestToken?: string | null) {
  const { data, error } = await supabase.rpc("complete_wild_faction_mission", { p_mission_id: missionId, p_guest_token: guestToken ?? null });
  if (error) throw new Error(error.message);
  return data as {
...
export async function beginWildSquad(supabase: SupabaseClient, gameId: string, guestToken?: string | null) { const { data, error } = await supabase.rpc("begin_wild_squad", { p_game_id: gameId, p_guest_token: guestToken ?? null }); if (error) throw new Error(error.message); return data as string; }
export async function getMyWildSquadState(supabase: SupabaseClient, gameId: string, guestToken?: string | null) { const { data, error } = await supabase.rpc("get_my_wild_squad_state", { p_game_id: gameId, p_guest_token: guestToken ?? null }); if (error) throw new Error(error.message); if (!data) return null; const squad = data as WildSquadState; return { ...squad, member_count: Number(squad.member_count), formation_progress: Number(squad.formation_progress), members_needed: Number(squad.members_needed) }; }
export async function createWildSquadToken(supabase: SupabaseClient, gameId: string, guestToken?: string | null) { const { data, error } = await supabase.rpc("create_wild_squad_token", { p_game_id: gameId, p_guest_token: guestToken ?? null }); if (error) throw new Error(error.message); return data as { token: string; qr_payload: string; short_code: string; expires_at: string }; }
export async function redeemWildSquadToken(supabase: SupabaseClient, gameId: string, value: string, guestToken?: string | null) { const { data, error } = await supabase.rpc("redeem_wild_squad_token", { p_game_id: gameId, p_token_or_code: value, p_guest_token: guestToken ?? null }); if (error) throw new Error(error.message); const result = data as { status: WildSquadFormationStatus; squad_id?: string; member_count?: number; formed?: boolean; just_formed?: boolean; room_id?: string }; if (result.just_formed) requestPushDispatch(supabase, result.room_id); return result; }
export async function getMyWildSquadMissionState(supabase: SupabaseClient, missionId: string, guestToken?: string | null) { const { data, error } = await supabase.rpc("get_my_wild_squad_mission_state", { p_mission_id: missionId, p_guest_token: guestToken ?? null }); if (error) throw new Error(error.message); const state = data as WildSquadMissionState; return { ...state, progress: Number(state.progress), required_progress: Number(state.required_progress), personal_progress: Number(state.personal_progress) }; }
export async function getWildSquadsOverview(supabase: SupabaseClient, gameId: string) { const { data, error } = await supabase.rpc("get_wild_squads_overview", { p_game_id: gameId }); if (error) throw new Error(error.message); return ((data ?? []) as WildSquadOverview[]).map((item) => ({ ...item, member_count: Number(item.member_count), missions_completed: Number(item.missions_completed) })); }
