import { BASIC_BALLOON, DEV_SPAWN_MAX_SECONDS, DEV_SPAWN_MIN_SECONDS, MANUAL_TAP_DAMAGE, ROOM_MAX_HEALTH } from "./constants";
import { getCellCenter, getLaneCell, isTraversalBlocked, SPAWN_LANES } from "./grid";
import { findPathToCeiling } from "./pathfinding";
import type { Balloon, BalloonDamageResult, BalloonRoom, BalloonSimulationEvent, PathBias, SpawnLane } from "./types";
...
export function createBalloonRoom(id: string): BalloonRoom {
  return { id, health: ROOM_MAX_HEALTH, maxHealth: ROOM_MAX_HEALTH, balloons: [], walls: [], wallRevision: 0, width: 1, height: 1 };
}
...
export function damageBalloon(room: BalloonRoom, balloonId: string, damage = MANUAL_TAP_DAMAGE): BalloonDamageResult | null {
