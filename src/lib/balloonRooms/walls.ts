import { MAX_HORIZONTAL_SUPPORT_DISTANCE, MAX_WALL_SEGMENTS } from "./constants";
import { getLaneCell, isValidWallEdge, SPAWN_LANES } from "./grid";
import { findPathToCeiling } from "./pathfinding";
import type { BalloonRoom, WallSegment, WallValidationResult } from "./types";
...
export function getUnsupportedHorizontalWalls(walls: WallSegment[]): WallSegment[] {
  ...
  return horizontalWalls.filter((wall) => {
    ...
    const queue: { x: number; distance: number }[] = [];
