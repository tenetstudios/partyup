import { ENTRY_LANE_COLUMNS, GRID_COLUMNS, GRID_ROWS } from "./constants";
import type { GridCell, SpawnLane, WallOrientation, WallSegment } from "./types";
...
export function getCellCenter(cell: GridCell): { x: number; y: number } {
  return { x: (cell.column + 0.5) / GRID_COLUMNS, y: (cell.row + 0.5) / GRID_ROWS };
}
