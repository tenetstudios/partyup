...
export function floatActionFromIntent(
  ...
  if (intent.actionType === "PLACE_WALL") {
    ...
    return { type: "PLACE_WALL", actorPlayerId, wall: createWallSegment(room.id, orientation as WallOrientation, integer(payload.gridX, "wall grid X"), integer(payload.gridY, "wall grid Y")) };
  }
  ...
  if (intent.actionType === "SEND_BALLOON") {
    ...
    return { type: "SEND_BALLOON", actorPlayerId, targetPlayerId: actorPlayerId === "playerA" ? "playerB" : "playerA", balloonType: balloonType as BalloonType, lane: lane as SpawnLane, sentAt: simulationTimeMs };
  }
  return { type: intent.actionType, actorPlayerId, wallSegmentId: stringValue(payload.wallSegmentId, "wall segment") } as FloatMatchAction;
}

export function advanceFloatStateTo(state: FloatMatchState, targetTimeMs: number) {
  const stepMs = SIMULATION_STEP_SECONDS * 1000;
  while (state.status === "active" && state.simulationTimeMs + stepMs <= targetTimeMs) updateFloatMatch(state, SIMULATION_STEP_SECONDS);
  return state;
}

export function reconcileFloatState(authoritativeState: FloatMatchState, pendingActions: readonly PendingFloatAction[], targetTimeMs: number) {
  ...
  for (const pending of pendingActions) {
    ...
    applyFloatMatchAction(state, floatActionFromIntent(state, pending.actorPlayerId, pending.intent, pending.simulationTimeMs));
  }
