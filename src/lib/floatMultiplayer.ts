...
function reportFloatError(operation: string, status: number | null, body: FloatErrorBody, fallback: string) {
  ...
  if (__DEV__) console.error(`[FLOAT ${operation.toUpperCase()} FAILED]`, { operation, status, code: body.code ?? null, postgres: message, details: body.details ?? null, hint: body.hint ?? null });
...
export function createFloatNetworkMatch() {
  return invokeFloat<{ match: FloatMatchRow }>({ operation: "create" });
}

export function joinFloatNetworkMatch(matchCode: string) {
  return invokeFloat<{ match: FloatMatchRow }>({ operation: "join", matchCode: matchCode.trim().toUpperCase() });
}

export function joinFloatPool(poolMode: FloatPoolMode, roomId?: string | null) {
  return invokeFloat<FloatPoolResult>({ operation: "poolJoin", poolMode, roomId: roomId ?? null, platform: "mobile", gameVersion: FLOAT_GAME_VERSION, coreVersion: FLOAT_CORE_VERSION });
}

export function getFloatPoolStatus() {
  return invokeFloat<FloatPoolResult>({ operation: "poolStatus" });
}

export function cancelFloatPool() {
  return invokeFloat<FloatPoolResult>({ operation: "poolCancel" });
}

export function readyFloatNetworkMatch(matchId: string) {
  return invokeFloat<{ match: FloatMatchRow }>({ operation: "ready", matchId });
}

export function syncFloatNetworkMatch(matchId: string) {
  return invokeFloat<{ match: FloatMatchRow }>({ operation: "sync", matchId });
}

export function heartbeatFloatNetworkMatch(matchId: string) {
  return invokeFloat<{ match: FloatMatchRow }>({ operation: "heartbeat", matchId });
}

export function persistFloatRealtimeActions(matchId: string, actions: FloatRealtimeAction[]) {
  return invokeFloat<{ persisted: number; lastSequence: number }>({ operation: "persistActions", matchId, actions });
}

export function checkpointFloatRealtimeMatch(
  matchId: string,
...
export function recoverFloatRealtimeMatch(matchId: string) {
