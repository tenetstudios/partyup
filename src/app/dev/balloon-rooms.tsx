...
function BalloonRoomsDevScreen() {
  ...
  const fieldHeight = Math.max(120, Math.min(430, windowHeight - 330));
  const launchQueue = snapshot.rooms.yours.attack.queue;
  const queueFull = launchQueue.length >= MAX_LAUNCH_QUEUE_SIZE;
  const insufficientSendCoins = snapshot.rooms.yours.economy.coins < BASIC_BALLOON_COST;
      ...
      <View style={styles.content}>
        ...
        <View style={styles.controlPanelsRow}>
          ...
          <View style={styles.controlPanel}>
            ...
            <Pressable onPress={sendBalloon} disabled={snapshot.rooms.opponent.health <= 0 || insufficientSendCoins || queueFull} style={[styles.sendButton, (snapshot.rooms.opponent.health <= 0 || insufficientSendCoins || queueFull) && styles.sendButtonDisabled]} accessibilityRole="button"><Text style={styles.sendButtonText}>SEND {BASIC_BALLOON_COST}</Text></Pressable>
            <Text numberOfLines={1} ellipsizeMode="tail" style={[styles.sendFeedback, (queueFull || insufficientSendCoins) && styles.feedbackInvalid]}>{queueFull ? "QUEUE FULL" : insufficientSendCoins ? `NEED ${BASIC_BALLOON_COST}` : sendFeedback || `+${BASIC_BALLOON_INCOME_GAIN} · Q ${launchQueue.length}/${MAX_LAUNCH_QUEUE_SIZE} ${launchQueue.map((queued) => queued.lane).join("›")}`}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  header: { height: 34, alignItems: "center", justifyContent: "center" },
  ...
  controlPanelsRow: { minHeight: 82, flexDirection: "row", alignItems: "stretch", gap: 5, marginTop: 5 }, controlPanel: { flex: 1, padding: 5, borderWidth: 1, borderColor: "rgba(221,194,255,0.18)", borderRadius: 8, backgroundColor: "rgba(23,16,32,0.96)" },
