...
export default function WildScreen() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView style={styles.page} contentContainerStyle={styles.content}>
    ...
    {!state ? <Text style={styles.muted}>Loading the Wild…</Text> : !state.game ? <View style={styles.panel}><Text style={styles.title}>THE WILD IS QUIET</Text><Text style={styles.muted}>The host has not started Into the Wild.</Text></View> : <>
      ...
      {state.assignment && squad && !isFormSquadMission && <View style={styles.squadPanel}><Text style={styles.squadEyebrow}>YOUR SQUAD</Text><Text style={styles.title}>{state.assignment.emoji} {squad.label}</Text><View style={styles.memberWrap}>{squad.members.map((member) => <Text key={member.identity_id} style={styles.memberChip}>{member.display_name}{member.is_you ? " (you)" : ""}</Text>)}</View><Text style={styles.muted}>{squad.member_count} {squad.member_count === 1 ? "member" : "members"}</Text>{squad.status === "active" && <Text style={styles.success}>SQUAD FORMED ✓</Text>}</View>}
      ...
      {state.assignment && state.game.status === "active" && <View style={styles.panel}>
        ...
        {state.mission ? <>
          ...
          {isSquadMission && !isFormSquadMission && <View style={styles.squadMission}><Text style={styles.squadEyebrow}>SQUAD MISSION · AGGREGATE</Text>{squadMissionState?.eligible ? <><Text style={styles.encounterProgress}>{Math.min(squadMissionState.progress, squadMissionState.required_progress)} / {squadMissionState.required_progress}</Text><Text style={styles.muted}>Your contribution: {squadMissionState.personal_progress} verified {squadMissionState.personal_progress === 1 ? "action" : "actions"}</Text></> : <Text style={styles.warning}>Form an active 3–5 player squad in the eligible faction to contribute.</Text>}</View>}
          {isFormSquadMission ? <View style={styles.squadMission}>
            {state.mission.eligible ? squad ? <>
              <Text style={styles.squadEyebrow}>FORM A SQUAD</Text>
              <Text style={styles.title}>{state.assignment.emoji} {squad.label}</Text>
              <View style={styles.memberWrap}>{squad.members.map((member) => <Text key={member.identity_id} style={styles.memberChip}>{member.display_name}{member.is_you ? " (you)" : ""}</Text>)}</View>
              <Text style={styles.encounterProgress}>{Math.min(squad.formation_progress, 2)} / 2</Text>
              {missionCompleted ? <Text style={styles.success}>SQUAD FORMED ✓ Influence awarded.</Text> : squad.status === "active" ? <Text style={styles.warning}>Your squad was already formed before this Mission began.</Text> : <>
                <Text style={styles.muted}>Find {squad.members_needed} more same-faction {squad.members_needed === 1 ? "player" : "players"}.</Text>
                {squad.can_add_members && <View style={styles.encounterActions}><TouchableOpacity style={styles.secondaryButton} onPress={() => setSquadMode("qr")}><Text style={styles.primaryText}>Show My Code</Text></TouchableOpacity><TouchableOpacity style={styles.primary} onPress={() => void openSquadScanner()}><Text style={styles.primaryText}>Scan Player</Text></TouchableOpacity></View>}
              </>}
            </> : <>
              <Text style={styles.requirement}>Find 2 other members of your faction.</Text>
              <Text style={styles.encounterProgress}>0 / 2</Text>
              <TouchableOpacity style={styles.primary} disabled={busy} onPress={() => void startSquad()}><Text style={styles.primaryText}>{busy ? "Starting…" : "Form a Squad"}</Text></TouchableOpacity>
            </> : <Text style={styles.warning}>This objective belongs to another faction.</Text>}
          </View> : state.mission.config.verification_type === "match_faction" ? <View style={styles.encounterBlock}>
